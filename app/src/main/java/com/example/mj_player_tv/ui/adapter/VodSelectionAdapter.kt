...
@UnstableApi
class VodSelectionAdapter(private val onClickListener: VodSelectionAdapter.OnClickListener, private val fragment: PlayMovieFragment, private val helpViewModel: HelpViewModel) : ListAdapter<TrackInfo, VodSelectionAdapter.ViewHolder>(
    ACCOUNT_COMPERATOR) {
    ...
    inner class ViewHolder(val binding: RvItemMovieSelectionBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(selection: TrackInfo) {
            binding.apply {
                Log.d("SELECTIONNAME", "${selection.trackName}")
